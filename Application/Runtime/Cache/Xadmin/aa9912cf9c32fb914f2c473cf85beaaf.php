...
<head>
    ...
    <title>代理审核-<?php echo (APP_NAME); ?></title>
    ...
    <div id="contentwrapper" class="contentwrapper">
        ...
        <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick">
            ...
            <?php if(empty($agentList)): ?><tr>
                    <td colspan="<?php echo ($cols); ?>">没有相关信息~！</td>
                </tr>
                <?php else: ?>
                <?php if(is_array($agentList)): $i = 0; $__LIST__ = $agentList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$agent): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td class="center"><?php echo ($agent['from_txt']); ?></td>
                        <td class="center"><?php echo (time_format($agent['dateline'])); ?></td>
                        <td class="center">
                        <?php if($agent['status'] == 1): ?>已处理
                            <?php else: ?>
                            未处理<?php endif; ?>
                        </td>
                        <td class="center">
                            ...
                            <?php else: ?>
                                <a class="stdbtn btn_lime del_confirm" title="标记为已通过" action_href="<?php echo U('Agent/agentExamineHandle', array('id'=>$agent['id']));?>">通过</a>&nbsp;&nbsp;<?php endif; ?>
                        </td>
